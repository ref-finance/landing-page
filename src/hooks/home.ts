...
export const useRefPrice = () => {
  ...
  useEffect(() => {
    axios.get<RefPrice>('https://indexer.ref-finance.fun/get-token-price?token_id=token.v2.ref-finance.near').then(res => {
      if (res.status === 200) {
        setData(res.data);
      } else {
...
export const useTokenBaeInfoList = () => {
  ...
  useEffect(() => {
    axios.get<Record<string, TokenBaseInfo>>('https://indexer.ref-finance.fun/list-token').then(res => {
      if (res.status === 200) {
